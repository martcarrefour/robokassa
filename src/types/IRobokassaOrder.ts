...
export interface IRobokassaOrder {
  /**
  Повторяющийся платеж (рекуррентный платеж).
  Указывает, что данный счет является частью серии повторяющихся платежей (например, подписка).
  Если параметр указан, Robokassa обрабатывает оплату как автоматическое списание по сохраненному сценарию.

  Пример использования:
  recurring: true — если вы запускаете автоматическое продление услуг, подписку и т.п.

  При использовании повторяющихся платежей требуется также параметр PreviousInvoiceID — номер первоначального счета в цепочке.
*/
  recurring?: boolean;

  /**
  Номер предыдущего счета в серии повторяющихся платежей.
  Указывается магазином при создании следующего платежа в рекуррентной серии.

  Обязателен, если выставляется повторный счет с флагом recurring.
  Значение должно быть идентификатором первоначального счета (InvoiceID), по которому пользователь впервые совершил оплату.

  Особенности:
  - Параметры PreviousInvoiceID и InvoiceID должны быть указаны обязательно;
  - InvoiceID должен быть сгенерирован магазином, не должен быть пустым или равным 0;
  - Не включается в формирование контрольной суммы SignatureValue;
  - Нельзя одновременно передавать IncCurrLabel, ExpirationDate и Recurring в одном запросе.
*/
