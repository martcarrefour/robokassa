...
export interface IRobokassaOrder {
  /**
    Этот параметр показывает, что выставление 
    данного счета будет повторяющимся
  */
  recurring?: boolean;
  /**
   * Магазин передает это параметр в вызове, направляемом на URL и указывает в нем номер счета первого платежа в серии повторяющихся платежей.
   * Кроме того, в этом вызове передаются обычные обязательные и необязательные параметры, со следующими исключениями:
   * 
